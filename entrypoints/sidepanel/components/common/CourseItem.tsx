...
export default function CourseItem({
  ...
  const ddayLabel = dday > 0 ? `D‑${dday}` : dday === 0 ? 'D‑day' : `마감+${Math.abs(dday)}`;
  ...
  return (
    ...
    >
      <div className="col-span-1 flex items-center justify-center w-full">{icon}</div>

      <div className="col-span-3 flex flex-col items-start justify-between w-full space-y-1 py-2">
        <div className="text-sm text-gray-500">
          <span
            className={`${badgeColor} text-white text-xs px-2 py-0.5 rounded-lg mr-2 font-semibold`}
          >
            {ddayLabel}
          </span>
          {courseTitle.split('-')[0]}
        ...
        <div className="text-xs text-gray-500">{formattedDue} 까지</div>
      </div>
    </li>
  );
}
